...
def init_langchain_model(llm: str, model_name: str, temperature: float = 0.0, max_retries=5, timeout=60, config=None,
                         **kwargs):
    """
    ...
    proxy = config.get('proxy', {}).get('address')
    llm_provider = llm_providers.get(llm, {})
    if llm == 'openai':
        ...
        return ChatOpenAI(
            api_key=llm_provider.get('api_key'),
            openai_api_base=llm_provider.get('api_base'),
            model_name=model_name,
            temperature=temperature,
            max_retries=max_retries, **kwargs)
        # return ChatTongyi(model_name=model_name, **kwargs)  # e.g., 'llama3'
    else:
